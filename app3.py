...
@st.cache_resource
def load_model():
    if os.path.exists('flight_model2.pkl'):
        try:
            model = joblib.load('flight_model2.pkl')
            return model
        except Exception as e1:
            try:
                with open('flight_model2.pkl', 'rb') as f:
                    model = pickle.load(f)
                return model
        ...
        st.error("Model file 'flight_model2.pkl' not found")
        return None
